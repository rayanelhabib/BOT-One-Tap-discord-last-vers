...
const RATE_LIMIT_WINDOW = 5000; // 5 secondes (ultra-rapide)
const RATE_LIMIT_MAX = 500; // 500 salons par fenêtre (pour dizaines de milliers d'utilisateurs)
const MAX_RETRIES = 3; // 3 tentatives (réduit pour plus de rapidité)
const OPERATION_TIMEOUT = 0; // 0ms (temps absolument maximal Discord API pour déplacement)
const WELCOME_MESSAGE_TIMEOUT = 4000; // 4.0 secondes (temps optimal pour message complexe)
const WELCOME_MESSAGE_ULTRA_FAST = true; // Mode ultra-rapide activé (1500ms)
...
const CHANNEL_DELETE_TIMEOUT = 800; // 0.8 seconde (temps réaliste Discord API pour suppression)
const MOVE_USER_TIMEOUT = 2000; // 2 secondes (temps optimal Discord API pour déplacement)
...
const CONFIG_CACHE_TTL = 2000; // 2 secondes (ultra-rapide)
const RATE_LIMIT_CACHE_TTL = 1000; // 1 seconde (ultra-rapide)
const CHANNEL_CACHE_TTL = 2000; // 2 secondes pour les salons (ultra-rapide)
const ULTRA_FAST_CACHE_TTL = 500; // 0.5 seconde pour les opérations critiques (ultra-rapide)
const CHANNEL_MONITOR_TTL = 8000; // 8 secondes pour le monitoring (ultra-rapide)
const ORPHAN_DETECTION_TTL = 30000; // 30 secondes pour détecter les orphelins (ultra-rapide)
    ...
    staticRow1.components.forEach(button => {
      if (button.data && button.data.custom_id) {
        const newButton = new ButtonBuilder()
          .setCustomId(button.data.custom_id.replace('PLACEHOLDER_', `vc_${button.data.custom_id.toLowerCase()}_${tempChannel.id}`))
    ...
    while (retries < MAX_RETRIES) {
      try {
        ...
        const channelCreatePromise = Promise.race([
          ...
          new Promise((_, reject) => 
            setTimeout(() => reject(new Error('Channel creation timeout')), 8000) // 8 secondes pour éviter les timeouts
          )
        ]);
        ...
        let welcomeSent = false;
          ...
          try {
            if (!tempChannel || !tempChannel.id) {
              ...
              const timeout = WELCOME_MESSAGE_ULTRA_FAST ? 1500 : WELCOME_MESSAGE_TIMEOUT;
            await Promise.race([
              tempChannel.send({
                flags: MessageFlags.IsComponentsV2,
                  components: [components.mainContainer],
    ...
    try {
      ...
      if (newState.channel?.id === config.createChannelId) {
        ...
        Promise.race([
          ...
          new Promise((_, reject) => 
            setTimeout(() => reject(new Error('Channel creation timeout')), 6000) // Temps optimal Discord API
          )
        ]).catch(error => {
      ...
      if (!global.cleanupInterval) {
          ...
          try {
            ...
            for (const [guildId, guild] of botModule.client.guilds.cache) {
              ...
              for (const [channelId, channel] of voiceChannels) {
                ...
                if (creatorId) {
                  console.log(`[CLEANUP] 🧹 Nettoyage de sécurité - Salon vide détecté: ${channel.name}`);
        ...
        }, 30000); // Toutes les 30 secondes (nettoyage de sécurité)
        
        console.log('[CLEANUP] ✅ Nettoyage de sécurité activé (toutes les 30 secondes)');
      }

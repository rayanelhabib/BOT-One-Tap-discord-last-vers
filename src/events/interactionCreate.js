...
• User management
    ...
    if (interaction.isStringSelectMenu() && interaction.customId === 'help-category-select') {
      ...
      // Import the help command to access the helper functions
      const helpCommand = require('../commands/prefix/help.js');

      // Get commands for the selected type
      const commands = helpCommand.getCommandsByType(selectedKey);

      if (commands.length > 0) {
        // Display all commands with descriptions
        await helpCommand.displayAllCommands(interaction, commands, selectedKey);
      } else {
        // Fallback to existing detailed pages for unsupported types
        const pageMapping = {
          'voice': 'commands',
          'blacklist': 'blacklist',
          'whitelist': 'whitelist',
        ...
        if (pageKey) {
          ...
          if (page) {
            ...
            // Fetch the navigation rows that are present in normal button navigation
    ...
    if (interaction.isButton() && interaction.customId === 'help_back_to_main') {
      ...
      const titleText = new TextDisplayBuilder()
        .setContent(`# <a:bitri9:1477098438876463306>  ${serverName} Help Commands`);
...
**<a:fleche:1477101417696333934> My Prefix:** \`.v\`
      ...
      const footerText = new TextDisplayBuilder()
        .setContent('<a:ta2ir:1477100378142015538> OneTab - Voice management | Use the menu below to navigate');
      ...
      const mainContainer = new ContainerBuilder()
        ...
        .addSeparatorComponents(separator)
        .addActionRowComponents(menuRow, buttonActionRow);
